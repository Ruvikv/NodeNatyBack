...
export default class UsersService {
    async find(email) {
        try {
            const [rows] = await conexionDB.query('SELECT * FROM usuarios WHERE correoElectronico = ?', [email]);
            return rows[0];
        } catch (error) {
    ...
    async findById(id) {
        try {
            ...
            const [rows] = await conexionDB.query('SELECT * FROM usuarios WHERE idUsuario = ?', [id]);
            if (rows.length === 0) {
    ...
    async delete(id) {
        try {
            const [result] = await conexionDB.query('DELETE FROM usuarios WHERE idUsuario = ?', [id]);
            return result.affectedRows > 0;
        } catch (error) {
