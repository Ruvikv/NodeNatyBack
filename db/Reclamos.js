...
export default class Reclamos {
    ...
    crear = async ({ asunto, descripcion, idReclamoTipo, idUsuarioCreador }) => {
        try {
            ...
            const [result] = await conexionDB.query(sql, [asunto, descripcion, idReclamoTipo, idUsuarioCreador]);
    ...
    //Método para modificar un reclamo (si lo necesitas en el futuro)
     ...
     async modificar(idReclamo, datosActualizacion) {
        ...
        try {
            const [result] = await conexionDB.query(query, values);
